...
def cvDrawBoxes(detections, img):
    ...
    if len(detections) > 0:  						# At least 1 detection in the image and check detection presence in a frame  
        ...
        if len(pos_dict) != 0:
            ...
            for k in range(n_clusters_):
                ...
                if n_member == 1:
                    ...
                    distance_to_center_2D = np.zeros(len(member))
                    ...
                    if np.sqrt(max_distance) < 20.0:       
                        cluster_den[k] = people_w/(pow(band, 2)*0.0001)
                  
                    else:
                        max_center[k] = max_distance
                        cluster_den[k] =  people_w*len(member)/(max_center[k]*0.0001)
        ...
        if risk < 1000:
            level = "low risk"
            cv2.putText(img, "risk: %s "% level + "(%s)"%str(round(risk, 4)), (10, 125), cv2.FONT_HERSHEY_SIMPLEX, 0.75, [0, 255, 50], 2)
        elif risk < 3000:
            level = "intermediate risk"
            cv2.putText(img, "risk: %s "% level + "(%s)"%str(round(risk, 4)), (10, 125), cv2.FONT_HERSHEY_SIMPLEX, 0.75, [255, 255, 0], 2)
        else:
            level = "high risk"
            cv2.putText(img, "risk: %s "% level + "(%s)"%str(round(risk, 4)), (10, 125), cv2.FONT_HERSHEY_SIMPLEX, 0.75, [255, 0, 0], 2) 
    return img
...
def YOLO():
    ...
    cap = cv2.VideoCapture("test_2.mp4")                             # Local Stored video detection - Set input video
