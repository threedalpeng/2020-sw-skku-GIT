...
def cvDrawBoxes(detections, img):
    #====================================================================
    # 3.1 get bounding box centroid for each person and mask detection.           
    #====================================================================
    red = (255, 0, 0)
    green = (0, 255, 0)
    blue = (0, 0, 255)
    if len(detections) > 0:  						# At least 1 detection in the image and check detection presence in a frame  
        person_detection = 0
        mask_weared_detection = 0
        mask_off_detection = 0
        ...
        for detection in detections:				# In this if statement, we filter all the detections for persons only
            ...
            name_tag = str(detection[0][1].decode())   # Coco file has string of all the names
            if name_tag == 'person':                
                ...
                cv2.putText(img,
                            ...
                            (pt1[0], pt1[1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                            green, 2)
                mask_weared_detection += 1

            elif name_tag == 'mask_off':
                x, y, w, h = detection[2][0],\
                            detection[2][1],\
                            detection[2][2],\
                            detection[2][3]      	# Store the center points of the detections
                ...
                cv2.putText(img,
                            ...
                            (pt1[0], pt1[1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                            blue, 2)
                mask_off_detection += 1


            objectId += 1 #Increment the index for each detection

            congestion = estimateCongestion(mask_weared_detection, mask_off_detection) 
                
        cv2.putText(img,
                    "Total people %s" % str(person_detection), (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    [0, 255, 50], 2)
        cv2.putText(img,
                    "Congestion %s" % str(congestion), (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    [0, 255, 50], 2)
    return img


def estimateCongestion(mask_on, mask_off):
